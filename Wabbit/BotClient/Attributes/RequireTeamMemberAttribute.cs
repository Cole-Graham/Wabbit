...
namespace Wabbit.BotClient.Attributes
{
    ...
    public class RequireTeamMemberAttribute : Attribute, IContextCheck
    {
        ...
        public async ValueTask<string?> ExecuteCheckAsync(CommandContext context)
        {
            try
            {
                ...
                // If it's an admin, they can bypass team member check
                ...
                return team.HasPlayer(context.User.Id)
                    ? null // Success 
                    : "You must be a member of this team to use this command.";
            }
            catch (Exception ex)
            {
                return $"Error checking team membership: {ex.Message}";
            }
        }
    }
}